...
dfolders_TrainingNames= dfolders_Training.name

for i = 1:length(dfolders_Training)
    x={dfolders_Training.name}
    PathToDataTraining(i,1)=char(x{1,i})+"/mondata.mat"
end
...
dfolders_TestNames= dfolders_Test.name

for i = 1:length(dfolders_Test)
    x={dfolders_Test.name}
    PathToDataTesting(i,1)=char(x{1,i})+"/mondata.mat"
end




FodlerName=dfolders_TrainingNames
AllMonodata1=load(fullfile(InputPath, PathToDataTraining(1,1)));
%AllMonodata1=load('DataSetsS1+0+0-noht1Good');
...
AllMonodata2=load(fullfile(InputPath, PathToDataTraining(2,1)));
%AllMonodata2=load('DataSetsS1+0+0-noht2Good');
...
AllMonodata3=load(fullfile(InputPath, PathToDataTraining(3,1)));
%AllMonodata3=load('DataSetsS1+0+0-noht3Good');
...
AllMonodata4=load(fullfile(InputPath, PathToDataTraining(4,1)));
%AllMonodata4=load('DataSetsS1+0+0-noht4Good');
...
% AllMonodata5=load('DataSetsS2+0+0-noht5.mat');
...
% % 
% AllMonodata6=load('DataSetsS2+0+0-noht6.mat');
...
% 
% AllMonodata7=load('DataSetsS2+0+0-noht7.mat');
...
% 
% AllMonodata8=load('DataSetsS2+0+0-noht8.mat');
...
% 
% AllMonodata9=load('DataSetsS2+0+0-noht9.mat');
...
% 
% %%%%%%%%%%% Picard01 %%%%%%%%%%
% AllMonodata10=load('DataSetsS2+0+0-noht10.mat');
...
% AllMonodata2=load('mondata_DataSetsS2+0+0-noht.mat');
% ht= ones(length(AllMonodata2.Tput) , 1);
% noth= zeros(length(AllMonodata2.Tput) , 1);
% nice= zeros(length(AllMonodata2.Tput) , 2); 
% data2= [ noth, nice , AllMonodata2.RunQ(:,1:4), AllMonodata2.Tput(:,1)];
% 
% 
% AllMonodata3=load('mondata_DataSetsS2+10+0-noht.mat');
% ht= ones(length(AllMonodata3.Tput) , 1);
% noth= zeros(length(AllMonodata3.Tput) , 1);
% nice= zeros(length(AllMonodata3.Tput) , 2); 
% nice(:,1)=10;
...
% 
% 
% AllMonodata4=load('mondata_DataSetsS2+0+0-noht-batch.mat');
% ht= ones(length(AllMonodata4.Tput) , 1);
% noth= zeros(length(AllMonodata4.Tput) , 1);
% nice= zeros(length(AllMonodata4.Tput) , 2); 
% data4= [ noth, nice, AllMonodata4.RunQ, AllMonodata4.Tput(:,1)];
...
% 
% 
% AllMonodata16=load('mondata_DataSetsS2+0+0+0+0-noht.mat');
...
% 
% AllMonodata17=load('mondata_DataSetsS2+15+10+5+0-noht.mat');
...
% 
% 
% AllMonodata18=load('mondata_DataSetsS2+15+10+5+15-noht.mat');
...
% 
% 
% AllMonodata19=load('mondata_DataSetsS2+15+10+10+15-noht.mat');
...
% 
% AllMonodata20=load('mondata_DataSetsS2+15+10+15+15-noht.mat');
...
OnlyTracesLetters= regexprep(Traces,'\d+',''); % to remove numbers from string
 for i = 1:size_Traces   % to add "-" for singl job
       i;
     if NumberOfJobs_Train == 2    
        if 0 == contains(OnlyTracesLetters(i,1),'-')
            OnlyTracesLetters(i,1)= strcat(OnlyTracesLetters(i,1),"-");
        end
     end
     
     if NumberOfJobs_Train == 4    
        if 0 == contains(OnlyTracesLetters(i,1),'-')
            OnlyTracesLetters(i,1)= strcat(OnlyTracesLetters(i,1),"---"); %three dash for more three jobs +single job
        end
     end 
...
newStrTrain=newStr;
newStrTrainnnnn=newStr;


unique_newStrTrain =unique(newStr)  % To creat an array without duplicate strings from the the original array
unique_newStrTrain(unique_newStrTrain(:,1)=="")=[] % To delet the empty row
unique_newStrTrain(unique_newStrTrain(:,1)=="0")=[] % To delet the empty row
xxxxx="555555555555555"
[ListOfJobsName, Column]= size(unique_newStrTrain)

for i=1:ListOfJobsName
    newStrTrain=strrep(newStrTrain,unique_newStrTrain(i,1),int2str(i));
end
...
TracesDouble = str2double(newStrTrain); 
[SizeR, SizeC]= size(TracesDouble);
RowNumbersOfJobsName_Train= zeros(SizeR, ListOfJobsName)  ; % ListOfJobsName is the number of jobs types

for i=1:SizeR
    RowNumbersOfJobsName_Train(i,TracesDouble(i,1))= RowNumbersOfJobsName_Train(i,TracesDouble(i,1)) +1;
    if (TracesDouble(i,2) ~= 0 )
        aaaa="44444444444"
        RowNumbersOfJobsName_Train(i,TracesDouble(i,2))= RowNumbersOfJobsName_Train(i,TracesDouble(i,2)) +1;
    end
end
data=[RowNumbersOfJobsName_Train, data]
...
%---------------------------------------------
%Testing data
%Test_File_mondata= "mondata_DataSetsS2+0+0+0+0-noht.mat"
%Test_File_mondata= "mondata_DataSetsS2+0+0+0+0-noht-z2.mat"
%Test_File_mondata= "mondata_DataSetsS2+15+10+5+0-noht.mat"
%Test_File_mondata= "mondata_DataSetsS2+15+10+5+15-noht.mat"
%Test_File_mondata= "mondata_DataSetsS2+15+10+10+15-noht.mat"
%Test_File_mondata= "mondata_DataSetsS2+15+10+15+15-noht.mat"
Test_File_mondata= "DataSetsS2+0+0+0+0-noht6-V2.mat"
Test_File_mondata= "DataSetsS1+0+0+0+0-noht_V3.mat"
Test_File_mondata="./DataSetsS1+0+0+0+0-noht/mondata.mat"
Test_File_mondata="DataSetsS1+0+0+0+0-noht1Good"
Test_File_mondata="DataSetsS1+0+0+0+0-noht1/mondata.mat"


AllMonodata_Test=load(fullfile(InputPath, 'DataSetsS1+0+0+0+0-noht/mondata.mat'));
...
data_Test=data_Test(:,end)


AllMonodata_Traces= [AllMonodata_Test.traces(1,:)]';


% The following code is to add the name of jpbs as a number(0 to 6). 0=no
% job
NumberOfJobs_Test=4;
size_Traces= length(AllMonodata_Traces);
Traces= string(AllMonodata_Traces);
OnlyTracesLetters= regexprep(Traces,'\d+',''); % to remove numbers from string
 for i = 1:size_Traces   % to add "-" for singl job
       i;
     if NumberOfJobs_Test == 2    
        if 0 == contains(OnlyTracesLetters(i,1),'-')
            OnlyTracesLetters(i,1)= strcat(OnlyTracesLetters(i,1),"-");
        end
     end
     
     if NumberOfJobs_Test == 4    
        if 0 == contains(OnlyTracesLetters(i,1),'-')
            OnlyTracesLetters(i,1)= strcat(OnlyTracesLetters(i,1),"---"); %thre dash for more thre jobs +single job
        end
     end 
     
     if NumberOfJobs_Test == 5    
        if 0 == contains(OnlyTracesLetters(i,1),'-')
            OnlyTracesLetters(i,1)= strcat(OnlyTracesLetters(i,1),"----"); %thre dash for more thre jobs +single job
        end
     end 
     
     if NumberOfJobs_Test == 6    
        if 0 == contains(OnlyTracesLetters(i,1),'-')
            OnlyTracesLetters(i,1)= strcat(OnlyTracesLetters(i,1),"-----"); %thre dash for more thre jobs +single job
        end
     end
     
     if NumberOfJobs_Test == 7    
        if 0 == contains(OnlyTracesLetters(i,1),'-')
            OnlyTracesLetters(i,1)= strcat(OnlyTracesLetters(i,1),"------"); %thre dash for more thre jobs +single job
        end
     end
     
     if NumberOfJobs_Test == 8    
        if 0 == contains(OnlyTracesLetters(i,1),'-')
            OnlyTracesLetters(i,1)= strcat(OnlyTracesLetters(i,1),"------"); %thre dash for more thre jobs +single job
        end
     end
 ...
 AAAAAAA="111111111111"

newStr = split(OnlyTracesLetters,"-")  % Split strings at delimiters
 AAAAAAA="22222222222"

%To convert any string with "" empty to be a zero
for j=1:NumberOfJobs_Test
    for i=1:size_Traces
        if (newStr(i,j) == "")
...
unique_newStrTest =unique(newStr)  % To creat an array without duplicate strings from the the original array
unique_newStrTest(unique_newStrTest(:,1)=="")=[] % To delet the empty row
unique_newStrTest(unique_newStrTest(:,1)=="0")=[] % To delet the empty row
xxxxx="555555555555555"
[ListOfJobsName_Test, Column_Test]= size(unique_newStrTest)

for i=1:ListOfJobsName_Test
    newStrTest=strrep(newStrTest,unique_newStrTest(i,1),int2str(i));
end
...
TracesDoubleTest = str2double(newStrTest);
[SizeR, SizeC]= size(TracesDoubleTest);
RowNumbersOfJobsName_Test= zeros(SizeR, 6);   % 6 is the number of jobs types

for i=1:SizeR
    RowNumbersOfJobsName_Test(i,TracesDoubleTest(i,1))= RowNumbersOfJobsName_Test(i,TracesDoubleTest(i,1)) +1;
    if (TracesDoubleTest(i,2) ~= 0)  % More than one job
        for j=2:NumberOfJobs_Test
        RowNumbersOfJobsName_Test(i,TracesDoubleTest(i,j))= RowNumbersOfJobsName_Test(i,TracesDoubleTest(i,j)) +1;
        end
        %RowNumbersOfJobsName_Test(i,TracesDoubleTest(i,2))= RowNumbersOfJobsName_Test(i,TracesDoubleTest(i,2)) +1;
        %RowNumbersOfJobsName_Test(i,TracesDoubleTest(i,3))= RowNumbersOfJobsName_Test(i,TracesDoubleTest(i,3)) +1;
        %RowNumbersOfJobsName_Test(i,TracesDoubleTest(i,4))= RowNumbersOfJobsName_Test(i,TracesDoubleTest(i,4)) +1;
    end
end
data_Test=[RowNumbersOfJobsName_Test, data_Test];
